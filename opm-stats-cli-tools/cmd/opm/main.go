...
func (m *MatchSimulator) simulateMovement(actor *Player) {
	...
	if eventType == EventDistance {
		// Match game tracker format: walked, sprinted, swam, driven
		event["walked"] = rand.Float32() * 50000
		event["sprinted"] = rand.Float32() * 30000
		event["swam"] = rand.Float32() * 5000
		event["driven"] = rand.Float32() * 10000
	}
	m.sendEvent(event)
}
