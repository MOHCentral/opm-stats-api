...
## 2024-05-25 - [Hot Path Map Allocations and String Formatting]
**Learning:** Found that achievement checking functions and server status updates in the worker pool were re-allocating `map[string]int` on every single event, and using `fmt.Sprintf` for high-frequency string generation. In a high-throughput event ingestion system, this created significant GC pressure and CPU overhead (e.g. fmt.Sprintf ~300ns vs concat ~90ns).
**Action:** Always inspect hot loops for hidden allocations like map literals. Move immutable lookup tables to package-level variables. Use `strconv` and string concatenation instead of `fmt.Sprintf` in hot paths to avoid reflection and heap allocation overhead.
