...
## 2024-05-20 - [Performance] Replace fmt.Sprintf with concatenation
**Learning:** String concatenation and `strconv.Itoa` are preferred over `fmt.Sprintf` in hot paths (like Redis key generation or status values) to avoid interface reflection and heap allocation overhead, reducing allocations significantly.
**Action:** Always prefer native string concatenation for simple string building in hot paths.
