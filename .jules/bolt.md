...
## 2024-06-12 - [fmt.Sprintf in Hot Paths]
**Learning:** `fmt.Sprintf` uses reflection and causes heap allocations. In hot paths like Redis key generation in the worker, this creates significant GC pressure and overhead.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` in high-frequency functions.
