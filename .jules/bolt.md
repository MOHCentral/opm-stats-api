...
## 2024-05-03 - [Optimize bytes.ReplaceAll in hot paths]
**Learning:** `bytes.ReplaceAll` unconditionally allocates a new byte slice even if the search target is not present, which is extremely costly in high-throughput endpoints.
**Action:** Guard `bytes.ReplaceAll` with `bytes.IndexByte` in hot paths to completely eliminate allocations when the payload is clean.
