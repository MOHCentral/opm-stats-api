...
## $(date +%Y-%m-%d) - [Fast Path for Null Byte Sanitization in Event Ingestion]
**Learning:** Found that `bytes.ReplaceAll` was unconditionally allocating memory and iterating over the entire request body during event ingestion to strip null bytes. Since most payloads are clean (no null bytes), this caused unnecessary GC overhead and CPU cycles. Adding a fast-path check `bytes.IndexByte(body, 0) != -1` before executing `ReplaceAll` yielded a significant (~14x) performance improvement for clean payloads during micro-benchmarking.
**Action:** Before executing an expensive, allocation-heavy operation like `bytes.ReplaceAll` or `strings.ReplaceAll` on large inputs, use a fast, allocation-free check like `bytes.IndexByte` or `strings.Contains` to ensure the operation is actually necessary.
