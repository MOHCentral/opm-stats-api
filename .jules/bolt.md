...
## 2024-05-15 - Replace fmt.Sprintf with string concatenation in hot paths
**Learning:** `fmt.Sprintf` incurs reflection and heap allocation overheads which can be avoided in hot loops (like Redis key generation) by using string concatenation and `strconv.Itoa`. Micro-benchmarks showed a ~3-4x speedup and elimination of allocs/op for these operations.
**Action:** Prefer string concatenation and `strconv.Itoa` over `fmt.Sprintf` in high-frequency data ingestion and caching loops.
