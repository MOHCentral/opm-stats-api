...
## 2024-06-25 - [fmt.Sprintf in Hot Paths]
**Learning:** `fmt.Sprintf` is considerably slower (~3.5x) than string concatenation (`+`) combined with `strconv.Itoa` when constructing simple strings like Redis keys or IDs, particularly in hot loops and high-throughput ingestion paths. The overhead comes from parsing the format string and handling interface{} reflection for variadic arguments.
**Action:** Use direct string concatenation and `strconv` functions for basic string formatting in performance-critical paths to significantly reduce GC pressure and execution time.
