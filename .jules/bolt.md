...
## 2024-05-18 - String concatenation beats fmt.Sprintf in hot paths
**Learning:** Replacing `fmt.Sprintf` with string concatenation and `strconv.Itoa` in hot paths like Redis key generation avoids interface reflection and heap allocation overhead, substantially reducing allocations.
**Action:** Use string concatenation + `strconv` over `fmt.Sprintf` for building short dynamic strings in tight loops.
