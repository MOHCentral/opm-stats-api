...
## 2024-05-27 - [Avoid fmt.Sprintf in Hot Path Strings]
**Learning:** `fmt.Sprintf` incurs reflection and interface boxing overhead, leading to slower execution times and higher memory allocations, especially in hot paths like the `updateServerStatus` worker function.
**Action:** Use native string concatenation (e.g., `+`) combined with type-specific formatters like `strconv.Itoa` or `strconv.FormatInt` instead of `fmt.Sprintf` for constructing simple strings in high-frequency loops.
