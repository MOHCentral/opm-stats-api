...
## 2024-05-25 - [Hidden Allocations in Hot Paths]
**Learning:** Found that using `fmt.Sprintf` for key generation and `uuid.MustParse` with constant strings inside event processing hot loops (like `convertToClickHouseEvent`, `updateServerStatus`, and achievement checks) creates significant unnecessary allocations and GC pressure. Replacing these with string concatenation/`strconv` and the built-in `uuid.Nil` eliminated these allocations completely.
**Action:** Avoid `fmt.Sprintf` for simple string formatting in hot paths, using `strconv` and string concatenation instead. For UUID zero-values, use `uuid.Nil` rather than parsing strings.
