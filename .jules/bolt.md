...
## 2025-01-20 - [Optimize string formatting]
**Learning:** `fmt.Sprintf` is slower and allocates more memory compared to simple string concatenation and `strconv.Itoa`.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` in hot paths like event processing and stats increments.
