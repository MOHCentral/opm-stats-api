...
## 2024-05-31 - [String Format Overhead in Hot Paths]
**Learning:** Used string concatenation and `strconv.Itoa` instead of `fmt.Sprintf` for Redis key and value generation in worker hot paths. This resulted in a ~3x-5x speedup and significant allocation reduction.
**Action:** When repeatedly generating keys or values in ingestion workers, avoid `fmt.Sprintf`. Prefer explicit string concatenation combined with `strconv` for numbers.
