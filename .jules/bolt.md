...
## 2024-11-20 - [Constant String parsing]
**Learning:** `uuid.MustParse("00000000-0000-0000-0000-000000000000")` inside a hot loop is very inefficient and incurs string parsing overhead, replacing it with the built-in `uuid.Nil` eliminated parsing overhead and memory allocations.
**Action:** When replacing all-zero UUID strings in hot loops, use the built-in `uuid.Nil` variable instead of `uuid.MustParse` to eliminate parsing overhead.
