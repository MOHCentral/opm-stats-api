...
## 2024-04-12 - String concatenation in hot paths
**Learning:** String formatting using `fmt.Sprintf` uses reflection which incurs unnecessary allocation overhead and CPU cycles when executed in high frequency hot paths (like game event processing). Using basic string concatenation along with `strconv.Itoa` can reduce allocation overhead and is significantly faster.
**Action:** Always prefer basic string concatenation and `strconv` functions over `fmt.Sprintf` when constructing dynamic keys or strings in execution-critical loops. Ensure required imports (like `strconv`) are added.
