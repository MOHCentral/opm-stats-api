...
## 2024-05-14 - Optimize Redis match outcome lookups
**Learning:** Pipelined `HGet` lookups in a loop for multiple fields allocate memory per command. Passing arrays to `HMGet` is cleaner and reduces overhead. Also, parsing integers via `fmt.Sscanf` induces reflection overhead and allocations compared to `strconv.ParseInt`.
**Action:** When pulling multiple fields from Redis hashes, use `HMGet`. When parsing strings to ints in hot loops, use `strconv`.
