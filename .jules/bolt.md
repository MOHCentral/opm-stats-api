...
## 2024-06-02 - [Optimize Redis key generation]
**Learning:** Using `fmt.Sprintf` to generate Redis keys in hot loops (like `incrementPlayerStat` and `checkKillAchievements`) causes significant interface reflection and allocation overhead.
**Action:** Prefer string concatenation and `strconv.Itoa` over `fmt.Sprintf` in high-frequency hot paths to minimize allocations and execution time.
