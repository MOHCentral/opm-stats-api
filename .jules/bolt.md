...
## 2025-02-24 - Optimize string parsing in hot loops
**Learning:** Using `fmt.Sscanf` or `strings.Split` for simple string parsing in high-frequency functions (like parsing Redis server status) causes significant allocation and reflection overhead.
**Action:** Replaced standard formatters with manual `strings.IndexByte`, `strings.HasPrefix`, and `strconv.Atoi` to achieve zero-allocation string parsing, yielding ~55x speedups.
