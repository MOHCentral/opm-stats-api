...
## 2025-02-14 - String Concatenation vs fmt.Sprintf in Hot Paths
**Learning:** Using `fmt.Sprintf` in high-frequency event processing loops (like Redis key generation) introduces significant reflection and heap allocation overhead compared to string concatenation with `strconv`.
**Action:** Always prefer basic string concatenation and `strconv.Itoa` over `fmt.Sprintf` in hot loops to avoid unnecessary memory allocations and improve throughput.
