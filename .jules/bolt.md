...
## 2024-06-04 - [Achievement map allocation overhead]
**Learning:** Allocating maps inside high-frequency event processor methods (like checking milestones) causes substantial GC pressure and execution delays. Additionally, `fmt.Sprintf` incurs reflection and allocation overhead in string formatting for Redis keys.
**Action:** Extract maps to package-level immutable variables and replace `fmt.Sprintf` with string concatenation combined with `strconv.Itoa` to eliminate allocations in hot paths.
