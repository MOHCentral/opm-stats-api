...
## 2024-05-25 - [Optimize `bytes.ReplaceAll` in Hot Path]
**Learning:** `bytes.ReplaceAll` unconditionally allocates a new slice, even if the search target isn't found. In the event ingestion handler, this caused unnecessary allocations on every clean payload.
**Action:** Guard string and bytes functions that always allocate (like `ReplaceAll` or `Trim`) with an `Index` or `Contains` check (e.g., `bytes.IndexByte(..., 0) != -1`) to avoid allocations for clean data.
