...
**Learning:** `fmt.Sprintf` incurs reflection and significant heap allocation overhead compared to direct string concatenation and `strconv.Itoa`. In high-throughput event processing loops (like Redis key generation and status string building), this allocation overhead creates measurable GC pressure.
**Action:** Replace `fmt.Sprintf` with string concatenation (`+`) and `strconv.Itoa` for simple strings in hot paths to minimize allocations.
