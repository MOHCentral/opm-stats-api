...
## 2024-05-25 - [Hot Path Redis Key Generation Allocations]
**Learning:** `fmt.Sprintf` is extremely slow and allocates on the heap in high-throughput hot paths (like `checkStreak`, `checkMultikillAchievement`, `incrementPlayerStat`) within the event processing worker pool. Every `fmt.Sprintf` call requires interface{} reflection and allocates memory.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` for dynamic string building when speed is critical. This approach allocates significantly less memory and performs 3-4x faster.
