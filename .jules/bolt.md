...
## 2023-10-27 - Guard bytes.ReplaceAll
**Learning:** Calling `bytes.ReplaceAll` to strip null bytes on clean payloads causes unnecessary slice allocation and copying overhead.
**Action:** Always guard unconditional stripping functions like `bytes.ReplaceAll` with a fast search using `bytes.IndexByte(data, target) != -1` to prevent allocations on clean paths.
