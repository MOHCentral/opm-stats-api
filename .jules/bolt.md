...
## 2024-05-24 - [Optimizing bytes.ReplaceAll]
**Learning:** Guarding `bytes.ReplaceAll` with `bytes.IndexByte` for clean payloads avoids unnecessary heap allocations and slice copying.
**Action:** Always check if characters exist before using heavy replace functions in hot paths.
