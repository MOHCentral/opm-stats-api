...
## 2024-05-24 - [Avoid fmt.Sprintf in hot paths for string concatenation]
**Learning:** `fmt.Sprintf` incurs reflection and memory allocation overhead. In hot paths like Redis key generation or logging formatting where simple variables (strings, integers) are combined, using string concatenation and `strconv.Itoa` reduces memory allocations significantly.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` for simple strings in hot execution loops to minimize allocation latency and improve throughput.
