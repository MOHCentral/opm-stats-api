...
## 2024-05-25 - [Hot Path String Parsing Allocation]
**Learning:** Calling `uuid.MustParse("constant-string")` inside a hot loop (like event ingestion processing) forces Go to allocate memory and re-parse the string into a UUID struct every single time. Micro-benchmarks confirmed this adds ~50ns overhead per call.
**Action:** Always pre-parse static identifiers into package-level variables during initialization rather than parsing them on the fly in hot paths.
