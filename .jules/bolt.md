...
## 2024-05-25 - [Formatting in Hot Paths]
**Learning:** Using `fmt.Sprintf` in high-frequency event ingestion hot paths (like Redis key generation) introduces reflection and allocation overhead.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` in worker hot paths for a ~2x-2.5x speedup and reduced GC pressure.
