...
## 2024-05-25 - [UUID Parsing in Hot Loops]
**Learning:** Inside `convertToClickHouseEvent` (a hot path for event processing), `uuid.MustParse("00000000-0000-0000-0000-000000000000")` was being called to generate a consistent namespace for invalid match IDs. This string parsing introduces unnecessary CPU overhead (about 50ns/op). Using the built-in `uuid.Nil` variable eliminates this parsing overhead entirely (reducing it to < 1ns/op).
**Action:** When replacing all-zero UUID strings in hot loops, use the built-in `uuid.Nil` variable instead of `uuid.MustParse` to eliminate string parsing overhead.
