...
// GetPlayerPredictions returns predictions for a player
func (h *Handler) GetPlayerPredictions(w http.ResponseWriter, r *http.Request) {
	guid := chi.URLParam(r, "guid")
	predictions, err := h.prediction.GetPlayerPredictions(r.Context(), guid)
	if err != nil {
		h.logger.Errorw("Failed to get predictions", "error", err)
		h.errorResponse(w, http.StatusInternalServerError, "Internal error")
		return
	}
