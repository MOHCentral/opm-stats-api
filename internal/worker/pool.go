...
func (p *Pool) updateServerStatus(ctx context.Context, event *models.RawEvent) {
	...
	statusStr := "players:" + strconv.Itoa(event.PlayerCount) + ",map:" + event.MapName + ",gametype:" + event.Gametype // OPTIMIZATION: Use string concatenation and strconv over fmt.Sprintf to prevent reflection and allocation overhead in hot path
