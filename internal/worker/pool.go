...
func (p *Pool) processBatch(batch []Job) error {
	...
	for _, job := range batch {
		...
		p.fillClickHouseEvent(&chEvent, event, job.RawJSON, job.Timestamp)
...
func (p *Pool) fillClickHouseEvent(ch *models.ClickHouseEvent, event *models.RawEvent, rawJSON string, receivedAt time.Time) {
