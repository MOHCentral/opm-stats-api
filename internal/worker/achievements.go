...
// achievementMilestones are mapped to package-level variables
// to prevent allocating map objects on every execution
var (
	...
	tankDestroyerMilestones = map[string]int{
...
func (w *AchievementWorker) checkCombatAchievements(smfID int64, event *models.RawEvent) {
	...
	if w.logger.Desugar().Core().Enabled(zap.DebugLevel) {
		w.logger.Debugw("Checking milestones", "totalKills", totalKills, "milestoneCount", len(killerMilestones))
	}
...
func (w *AchievementWorker) checkVehicleAchievements(smfID int64, event *models.RawEvent) {
	...
	for slug, threshold := range tankDestroyerMilestones {
