...
func (w *AchievementWorker) checkStreak(smfID int64, event *models.RawEvent) {
	...
	// ⚡ Bolt: Use string concatenation instead of fmt.Sprintf for Redis keys to prevent reflection and heap allocations
...
func (w *AchievementWorker) incrementPlayerStat(smfID int, statName string) int {
	// ⚡ Bolt: Use string concatenation + strconv instead of fmt.Sprintf for Redis keys to prevent reflection and heap allocations
