...
func (w *AchievementWorker) checkStreak(smfID int64, event *models.RawEvent) {
	...
	// Optimized: Replaced fmt.Sprintf with string concatenation to prevent memory allocation overheads in hot paths.
...
func (w *AchievementWorker) incrementPlayerStat(smfID int, statName string) int {
	// Optimized: Replaced fmt.Sprintf with string concatenation and strconv.Itoa to prevent memory allocation overheads in hot paths.
