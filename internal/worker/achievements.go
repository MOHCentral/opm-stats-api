...
func (w *AchievementWorker) checkStreak(smfID int64, event *models.RawEvent) {
	...
	// Optimized: use string concat instead of fmt.Sprintf to avoid heap allocations in hot path
...
func (w *AchievementWorker) incrementPlayerStat(smfID int, statName string) int {
	// Optimized: use string concat + strconv.Itoa instead of fmt.Sprintf to avoid heap allocations
