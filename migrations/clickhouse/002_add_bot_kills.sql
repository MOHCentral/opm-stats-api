-- Migration: Add bot_kills column for Bot Slayer leaderboard
-- This adds tracking for bot kills separately from player kills

-- Step 1: Add the new column to the aggregation table
ALTER TABLE mohaa_stats.player_stats_daily ADD COLUMN IF NOT EXISTS bot_kills UInt64 DEFAULT 0;

-- Step 2: Drop and recreate the actor MV to include bot_kills
...
AS SELECT
    ...
    -- Combat (Actor side) - kills is player kills only for competitive fairness
...
-- Step 3: Backfill bot_kills from existing data 
-- This populates the new column with historical data
INSERT INTO mohaa_stats.player_stats_daily (day, player_id, player_name, kills, deaths, headshots, bot_kills, last_active)
SELECT 
    toStartOfDay(timestamp) AS day,
    actor_id AS player_id,
    ...
    max(timestamp) AS last_active
FROM mohaa_stats.raw_events
WHERE event_type = 'bot_killed' AND actor_id != '' AND actor_id != 'world'
GROUP BY day, actor_id;
